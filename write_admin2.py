...
# Read the current Admin.jsx (first 200 lines = everything before the dashboard tab content)
# We only need to ADD the Live Orders tab — patch the existing file

existing = open("simba-supermarket/src/pages/Admin.jsx", encoding="utf-8").read()

# Find where the navItems array is defined and add liveOrders to it
existing = existing.replace(
    '{ id: "settings", label: "Settings", icon: Settings },',
    '{ id: "settings", label: "Settings", icon: Settings },\n    { id: "live", label: t.liveOrders || "Live Orders", icon: Users },'
)

# Find the closing of the products tab and insert the live orders tab before settings tab
LIVE_ORDERS_TAB = r"""
          {/* ===== LIVE ORDERS TAB ===== */}
          {tab === 'live' && (
            <LiveOrdersTab orders={orders} updateOrderStatus={updateOrderStatus} t={t} dm={dm} card={card} txt={txt} sub={sub} />
          )}

"""

# Insert before the settings tab
existing = existing.replace(
    "          {/* ===== SETTINGS TAB ===== */}",
    LIVE_ORDERS_TAB + "          {/* ===== SETTINGS TAB ===== */"
)

open("simba-supermarket/src/pages/Admin.jsx", "w", encoding="utf-8").write(existing)
print(f"Admin patched: {len(existing.splitlines())} lines")

# Now write the LiveOrdersTab component into a separate file
LIVE_COMPONENT = r"""
...
open("simba-supermarket/src/pages/LiveOrders.jsx", "w", encoding="utf-8").write(LIVE_COMPONENT)
print(f"LiveOrders: {len(LIVE_COMPONENT.splitlines())} lines")
