    ...
    <div className={'min-h-screen flex ' + (dm ? 'bg-zinc-950' : 'bg-slate-50')}>
      <div className="hidden lg:flex flex-1 relative overflow-hidden">
        ...
        <div className="absolute inset-0 flex flex-col justify-center px-16">
          ...
          <p className="text-zinc-300 text-base">11 branches across Kigali. 789+ products. Delivered to your door.</p>
        </div>
      </div>
      <div className={'flex flex-col justify-center px-6 py-12 w-full lg:w-[440px] lg:flex-shrink-0 ' + (dm ? 'bg-zinc-900' : 'bg-white')}>
        <div className="max-w-sm mx-auto w-full">
          ...
          <p className={'text-sm mb-7 ' + (dm ? 'text-zinc-500' : 'text-zinc-500')}>Welcome back to Simba Supermarket</p>

          {err && <div className="bg-red-50 border border-red-200 text-red-600 text-sm px-4 py-3 rounded-xl mb-4">{err}</div>}

          <form onSubmit={submit} className="space-y-4">
            <div>
              <label className={'block text-xs font-semibold mb-1.5 ' + (dm ? 'text-zinc-400' : 'text-slate-600')}>{t.email}</label>
              <input type="email" required value={form.email} onChange={e => setForm(f=>({...f,email:e.target.value}))} className={inp} />
            </div>
            <div>
              <label className={'block text-xs font-semibold mb-1.5 ' + (dm ? 'text-zinc-400' : 'text-slate-600')}>{t.password}</label>
              <div className="relative">
                ...
                <button type="button" onClick={() => setShow(!show)} className={'absolute right-3 top-1/2 -translate-y-1/2 ' + (dm ? 'text-zinc-500' : 'text-zinc-400')}>
          ...
          <div className={'mt-5 p-3.5 rounded-xl text-xs text-center ' + (dm ? 'bg-zinc-800 text-zinc-400' : 'bg-slate-50 text-zinc-500')}>
