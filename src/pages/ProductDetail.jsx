  ...
  return (
    <div className={'min-h-screen ' + (dm ? 'bg-slate-950 text-white' : 'bg-slate-50 text-slate-900')}>
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-8">
        <Link to="/shop" className={'inline-flex items-center gap-2 mb-6 text-sm font-medium hover:text-orange-500 transition-colors ' + (dm ? 'text-slate-400' : 'text-slate-600')}>
          <ArrowLeft size={16} /> {t.backToShop}
        </Link>

        <div className={'rounded-2xl overflow-hidden border ' + card}>
          <div className="grid md:grid-cols-2">
            <div className={'aspect-square bg-slate-100 ' + (dm ? 'dark:bg-slate-800' : '')}>
              {!imgErr
            ...
            <div className="p-8 flex flex-col justify-center">
              <div className="flex items-center gap-2 mb-4">
                <span className={'text-xs px-3 py-1 rounded-full font-medium ' + (dm ? 'bg-slate-800 text-slate-300' : 'bg-slate-100 text-slate-600')}>
              ...
              <div className="mb-6">
                ...
                <span className={'text-base ml-1 ' + (dm ? 'text-slate-400' : 'text-slate-500')}>RWF</span>
              </div>
              <div className={'flex items-center gap-2 mb-5 text-sm ' + (dm ? 'text-slate-400' : 'text-slate-600')}>
                <Package size={14} /> Unit: {product.unit}
              </div>
              <div className="flex items-center gap-4 mb-6">
                <span className={'text-sm font-semibold ' + (dm ? 'text-slate-300' : 'text-slate-700')}>{t.quantity}:</span>
                <div className={'flex items-center rounded-xl border ' + (dm ? 'border-slate-700' : 'border-slate-200')}>
                  <button onClick={() => setQty(q => Math.max(1,q-1))} className={'w-9 h-9 flex items-center justify-center hover:text-orange-500 transition-colors ' + (dm ? 'text-slate-400' : 'text-slate-500')}><Minus size={14} /></button>
                  <span className={'w-10 text-center font-bold ' + (dm ? 'text-white' : 'text-slate-900')}>{qty}</span>
                  <button onClick={() => setQty(q => q+1)} className={'w-9 h-9 flex items-center justify-center hover:text-orange-500 transition-colors ' + (dm ? 'text-slate-400' : 'text-slate-500')}><Plus size={14} /></button>
                </div>
              </div>
              ...
              <div className={'mt-5 p-4 rounded-xl text-sm space-y-1.5 ' + (dm ? 'bg-slate-800' : 'bg-slate-50')}>
                <p className={dm ? 'text-slate-300' : 'text-slate-600'}>🚚 Free delivery on orders over 50,000 RWF</p>
                <p className={dm ? 'text-slate-300' : 'text-slate-600'}>💳 Pay with MTN MoMo, Airtel Money or Cash</p>
                <p className={dm ? 'text-slate-300' : 'text-slate-600'}>🔄 Easy returns within 7 days</p>
              </div>
            </div>
          </div>
        </div>
